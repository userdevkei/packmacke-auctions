        ...
        ->orderBy('priority', 'asc') // <-- sort by priority first
            ...
            ->map(function ($task) use ($priorities, $pColors) {
                ...
                return [
                    ...
                    'task_date' => $task->task_date ? Carbon::createFromTimestamp($task->task_date, config('app.timezone'))->format('d/m/y') : null,
                ];
            });
        return $tasks;
    }
...
// Pending on this day (created before or on this day, not completed)
            $pending[] = (clone $query)
                ->whereIn('status', [self::STATUS_NOT_STARTED, self::STATUS_IN_PROGRESS])
                ->where('created_at', '<=', Carbon::createFromTimestamp($dateEnd, config('app.timezone')))
                ->count();
