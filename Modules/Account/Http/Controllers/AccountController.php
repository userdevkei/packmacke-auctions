...
class AccountController extends Controller
{
    ...
    public function downloadInvoice($id)
    {
        ...
        $values = $inv->first();
        $invDate = Carbon::createFromTimestamp($values['date_invoiced'], config('app.timezone'))->format('Y-m-d');
        ...
        $narration = $values['customer_message'] . ' INVOICE TO BE SETTLED BY OR BEFORE ' . Carbon::createFromTimestamp($values['due_date'], config('app.timezone'))->format('D, d-m-Y');
        ...
        $invoice->setValue('invDate', Carbon::createFromTimestamp($values['date_invoiced'], config('app.timezone'))->format('d/m/Y'));
        $invoice->setValue('dueDate', $values['type'] == 1 ? Carbon::createFromTimestamp($values['due_date'], config('app.timezone'))->format('d/m/Y') : $values['inv_reference']);
    ...
    public function downloadPurchaseReceipt($id)
    {
        ...
        $invDate = Carbon::createFromTimestamp($payment->date_received, config('app.timezone'))->format('d-m-Y');
    ...
    public function downloadPurchaseVoucher($id)
    {
        ...
        $type = 'PURCHASE';
        $narration = 'INVOICE TO BE SETTLED BY OR BEFORE ' . Carbon::createFromTimestamp($values->due_date, config('app.timezone'))->format('D, d-m-Y');
            ...
            if ($request->dateFrom !== null && $request->dateTo !== null || $request->dateFrom == null && $request->dateTo !== null || $request->dateFrom !== null && $request->dateTo == null) {
                ...
                if ($openingBalance != 0) {
                    $opening = [
                        ...
                        'description' => 'Balance as of ' . Carbon::createFromTimestamp($startDate, config('app.timezone'))->format('d-m-Y'),
                        'type' => $account->type
                    ];
    ...
    public function downloadPettyCashPayment($id)
    {
        ...
        $financial =  FinancialYear::where('year_starting', '<=', Carbon::createFromTimestamp($payment['date_invoiced'], config('app.timezone'))->format('Y-m-d'))->where('year_ending', '>=', Carbon::createFromTimestamp($payment['date_invoiced'])->format('Y-m-d'))->first();
        ...
        $amount = $payment['currency_name'] . ' ' . $amount . ' Only';
        $invDate = Carbon::createFromTimestamp($payment->date_invoiced, config('app.timezone'))->format('d/m/Y');
...
if (!is_null($deliveryType)) {
    ...
    public function viewShipments(Request $request)
    {
        ...
        $availableMonths = collect()
            ...
            ->map(function ($date) {
                return Carbon::createFromTimestamp($date, config('app.timezone'))->format('Y-m');
            })
            ->unique()
            ->sortDesc()
            ->values();
