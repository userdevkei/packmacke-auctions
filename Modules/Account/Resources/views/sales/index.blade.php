                    ...
                    <table class="table mb-0 table-bordered table-striped fs-sm" id="datatable">
                        ...
                        @foreach($invoices as $invoice)
                            <tr>
                                ...
                                <td> {{ \Carbon\Carbon::createFromTimestamp($invoice->date_invoiced, config('app.timezone'))->format('D, d/m/y') }} </td>
                                <td> {{ \Carbon\Carbon::createFromTimestamp($invoice->due_date, config('app.timezone'))->format('D, d/m/y') }} </td>
                                <td>
                                    @php
                                        $dueDateTimestamp = $invoice->due_date;
                                        $dateInvTimestamp = $invoice->date_invoiced;

                                            $dueDate = \Carbon\Carbon::createFromTimestamp($dueDateTimestamp, config('app.timezone'));
                                            $dateInv = \Carbon\Carbon::createFromTimestamp($dateInvTimestamp, config('app.timezone'));
