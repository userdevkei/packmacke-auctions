        ...
        <div class="card-body overflow-hidden p-lg-3">
            <div class="row align-items-center">
                <div class="tab-pane preview-tab-pane active" role="tabpanel" aria-labelledby="tab-dom-c3976e0e-38db-410e-861a-36d04a3a7494" id="dom-c3976e0e-38db-410e-861a-36d04a3a7494">
                    <form class="needs-validation" method="POST" action="{{ route('accounts.storeCreditNote', $invoice[0]->invoice_id) }}" novalidate>
                        @csrf
                                ...
                                <table class="table table-striped credit-note-table table-bordered fs-sm table-sm">
                                    ...
                                    @foreach($invoice as $item)
                                        <tr>
                                            ...
                                            <td>
                                                <input type="number" value="{{ $item->quantity }}" max="{{ $item->quantity }}" min="0" step="0.001" class="form-control form-control-sm new-qty" name="creditItems[{{ $item->invoice_item_id }}][credit_quantity]" data-rate="{{ $item->unit_price }}" data-tax="{{ $item->tax_rate == null ? 0 : $item->tax_rate }}" placeholder="Enter credit quantity" style="height: 3vh !important;"></td>
                                            <td><input type="number" step="0.001" max="{{ $item->unit_price }}" class="form-control form-control-sm new-rate" name="creditItems[{{ $item->invoice_item_id }}][credit_rate]" data-quantity="{{ $item->quantity }}" value="{{ $item->unit_price }}" placeholder="Enter new rate" style="height: 3vh !important;"></td>
                                            <input type="hidden" value="{{ $item->tax_id }}" name="creditItems[{{ $item->invoice_item_id }}][credit_tax]">
