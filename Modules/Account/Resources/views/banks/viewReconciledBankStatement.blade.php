                    ...
                    <table class="table mb-0 table-bordered table-striped" id="datatable">
                        ...
                        @foreach($statements as $statement)
                            <tr>
                                ...
                                <td>{{ \Carbon\Carbon::createFromTimestamp($statement->date_received, config('app.timezone'))->format('d-m-Y') }}</td>
                                <td>
                                    @if(auth()->user()->role_id == 7)
                                    <input type="date" class="form-control form-control-sm date-input" value="{{ $statement->bank_date ? Carbon\Carbon::createFromTimestamp($statement->bank_date, config('app.timezone'))->format('Y-m-d') : '' }}" data-id="{{ $statement->transaction_id }}">
                                    @else
                                    {{ Carbon\Carbon::createFromTimestamp($statement->bank_date, config('app.timezone'))->format('Y-m-d') }}
                                    @endif
                                </td>
                            </tr>
                          @endforeach
