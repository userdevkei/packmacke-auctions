        ...
        <div class="card-header">
            <div class="row flex-between-center">
                <div class="col-6 col-sm-auto d-flex align-items-center pe-0">
                    <h5 class="fs-9 mb-0 text-nowrap py-0 py-xl-0"> {{ $client->client_account_name }} (FY {{ Carbon\Carbon::parse($fy->year_starting)->format('Y') == Carbon\Carbon::parse($fy->year_ending)->format('Y') ? Carbon\Carbon::parse($fy->year_starting)->format('Y') : Carbon\Carbon::parse($fy->year_starting)->format('Y').'/'.Carbon\Carbon::parse($fy->year_ending)->format('y') }} ) </h5>
                </div>
                    ...
                    <div class="modal-content border-0">
                        ...
                        <div class="modal-body p-0">
                            <div class="rounded-top-3 bg-body-tertiary py-3 ps-4 pe-6">
                                <h5 class="mb-1" id="staticBackdropLabel">FILTER BY DATE {{ $fy->year_starting }}</h5>
                            </div>
                            <div class="p-4">
                                <div class="row">
                                    <form method="POST" action="{{ route('accounts.generateClientStatement') }}" target="_blank">
                                        <div class="row row-cols-sm-1 g-2">
                                            ...
                                            <div class="mb-2">
                                                ...
                                                <select class="form-control js-choice" name="reportType">
                                        ...
                                        <label class="text-danger">Include Opening Balance</label> <input type="checkbox" name="openingBalance" checked class="mx-2">
                                    ...
                                    @if($opBal !== null)
                                        <form method="POST" action="{{ route('accounts.updateOpeningBalance') }}">
                                            <div class="row row-cols-sm-1 g-2">
                                                ...
                                                <div class="mb-1">
                                                    <div class="form-floating mb-4">
                                                        <select class="form-select" name="balanceType" required>
                                                            <option value="">-- select balance type--</option>
                                                            <option value="1">DEBIT BALANCE</option>
                                                            <option value="2">CREDIT BALANCE</option>
                                                        </select>
                                                ...
                                                <div class="mt-2">
                                                    <div class="form-floating mb-4">
                                                        <input type="number" step="0.01" min="1" name="amountInvoice" class="form-control" placeholder="--" required>
                                            ...
                                            <div class="mt-2 d-flex justify-content-center">
                                                <button type="submit" class="btn btn-md btn-falcon-success col-md-8">ADD OPENING BALANCE</button>
                                            </div>
                ...
                <div class="tab-pane preview-tab-pane active" role="tabpanel" aria-labelledby="tab-dom-c3976e0e-38db-410e-861a-36d04a3a7494" id="dom-c3976e0e-38db-410e-861a-36d04a3a7494">
                    <table class="table mb-0 table-bordered table-striped" id="datatable">
                        ...
                        <tr>
                            ...
                            <th>{{ $client->type == 4 ? "LEDGER NAME" : "ACCOUNT NAME" }}</th>
                            <th>{{ $client->type == 4 ? "ACCOUNT REC'D/PAID" : ($client->type == 7 || $client->type == 8 ? 'LEDGER NAME' : ($client->type == 1 || $client->type == 2 || $client->type == 3 ? "LEDGER NAME" : "ACCOUNT NAME")) }}</th>
                            <th>DESCRIPTION</th>
                            <th>DEBIT</th>
                            <th>CREDIT</th>
                        </tr>
                        ...
                        @foreach($statements as $statement)
                            <tr>
                                ...
                                <td> {{ Carbon\Carbon::createFromTimestamp($statement->transaction_date, config('app.timezone'))->format('d/m/y') }} </td>
                                <td> {{ $statement->transaction_type }} </td>
                                <td> {{ $statement->transaction_number }} </td>
                                <td> {{ ucwords(strtolower($statement->client_account_name)) }} </td>
                                <td> {{ ucwords(strtolower($statement->ledger_name)) }} </td>
                                <td style="white-space: normal; word-break: break-word;"> {{ ucfirst(strtolower($statement->description)) }} </td>
                        ...
                        <tr class="text-center">
                            <td colspan="7" class="fw-bold text-center">TOTAL INVOICE</td>
                            <td colspan="1" class="fw-bold"> {{ number_format($totalDebit, 2, '.', ',') }}</td>
                            <td colspan="1" class="fw-bold"> {{ number_format($totalCredit, 2, '.', ',') }}</td>
                        </tr>
                        <tr>
                            <td colspan="7" class="fw-bold text-center"> BALANCE </td>
                            <td colspan="2" class="fw-bold text-center"> {{ $currency->currency_symbol }} {{ $totalDebit - $totalCredit < 0 ? '('.number_format($totalDebit - $totalCredit, 2, '.', ',').')' : number_format($totalDebit - $totalCredit, 2, '.', ',') }} </td>
                        </tr>
                    </table>
                </div>
            </div>
