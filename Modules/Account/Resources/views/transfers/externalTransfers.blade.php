                    ...
                    <table class="table mb-0 table-bordered table-striped" id="datatable">
                        ...
                        @foreach($transfers as $transfer)
                            <tr>
                                ...
                                <td>
                                    {!! $transfer->status === 0 || $transfer->status == 1 ? '<span class="badge bg-info"> Initiated <span>' : ($transfer->status == 2 ? '<span class="badge bg-dark"> Approved (Op) <span>' : ($transfer->status == 3 ? '<span class="badge bg-secondary"> Approved (Fin) <span>' : '<span class="badge bg-success"> Released <span>')) !!}
                                </td>
                                <td nowrap="">
                                    <div class="d-flex align-items-center">
                                        ...
                                        <div class="dropdown font-sans-serif position-static" >
                                            ...
                                            <div class="dropdown-menu dropdown-menu-end border py-0">
                                                <div class="py-2">
                                                    <a class="dropdown-item text-info" href="{{ route('accounts.viewExternalTransferDetails', base64_encode($transfer->delivery_number)) }}">View Transfer</a>
                                                    @if($transfer->buyer_name == null)
                                                        <a class="dropdown-item text-primary" href="{{ route('accounts.downloadExtraDelNote', base64_encode($transfer->delivery_number.':'.$transfer->lot)) }}" target="_blank">Download Transfer</a>
                                                    @else
                                                        @if (in_array(auth()->user()->role_id, [2, 3, 5, 7]))
                                                            <a class="dropdown-item text-danger" href="{{ route('accounts.downloadDelNote', base64_encode($transfer->delivery_number.':'.$transfer->lot)) }}" target="_blank">Download Del Note</a>
                                                        @endif
                                                    @endif
                                                </div>
                                            </div>
