        ...
        <div class="card-header">
            <div class="row flex-between-center">
                <div class="col-6 col-sm-auto d-flex align-items-center pe-0">
                    <h5 class="fs-9 mb-0 text-nowrap py-0 py-xl-0">Stocks Per Warehouse </h5>
                </div>
            ...
            <div class="row align-items-center">
                <div class="tab-pane preview-tab-pane active" role="tabpanel" aria-labelledby="tab-dom-c3976e0e-38db-410e-861a-36d04a3a7494" id="dom-c3976e0e-38db-410e-861a-36d04a3a7494">
                    <table class="table mb-0 table-bordered table-striped" id="datatable">
                        ...
                        <tbody>
                        @foreach($warehouses as $request)
                            <tr>
                                <td> {{ $loop->iteration }} </td>
                                <td> {{ $request->station_name }}</td>
                                <td> {{ number_format($request->total_packages, 0) }}</td>
                                <td> {{ number_format($request->total_weight, 2) }}</td>
                                <td> {{ number_format($request->percentage_weight, 2) }} % </td>
                                <td>
                                    <a class="link text-secondary m-2" data-bs-toggle="tooltip" data-bs-placement="left" title="View Clients Stock Analysis" href="{{ route('admin.clientStockPerWarehouse',$request->station_id) }}"> <span class="fas fa-folder-open"> </span> </a>
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
