                                ...
                                <td nowrap="">
                                    <div class="d-flex align-items-center">
                                        ...
                                        @if($transfer->status == 0)
                                            ...
                                            @else
                                                <a class="link text-info" data-bs-toggle="tooltip" data-bs-placement="left" title="SI pending submission for approval"><span class="fa-solid fa-spinner"></span></a>
                                            @endif
                                        @elseif($transfer->status == 1)
                                                    ...
                                                    <div class="modal-dialog modal-xl modal-dialog-centered mt-6" role="document">
                                                        <div class="modal-content border-0">
                                                            ...
                                                            <div class="modal-body p-0">
                                                                ...
                                                                <div class="p-4">
                                                                    <form class="form" method="POST" action="{{ route('clerk.updateShippingInstructionDetails', $transfer->shipping_id) }}">
                                                                        @csrf
                                            ...
                                            @else
                                                <a class="link text-danger" data-bs-toggle="tooltip" data-bs-placement="left" title="SI Updated, pending details update"><span class="fa-solid fa-spinner"></span></a>
                                            @endif
                                        @elseif($transfer->status == 2)
                                            @if($transfer->location_id == auth()->user()->station->location->location_id && auth()->user()->role_id == 3)
                                                <a class="link text-danger" data-bs-toggle="tooltip" data-bs-placement="left" title="Send SI for approval" onclick="return confirm('Are you sure you want to send this SI for approval?')" href="{{ route('clerk.updateShippingInstruction', $transfer->shipping_id) }}"><span class="fa-regular fa-paper-plane"></span></a>
                                            @else
                                        ...
                                        @elseif($transfer->status == 3)
                                            @if(auth()->user()->role_id == 2)
                                                <a class="link text-danger" data-bs-toggle="tooltip" data-bs-placement="left" title="SI pending confirmation and shipping" onclick="return confirm('Are you sure you want to approve this SI? This will mark SI as shipped')" href="{{ route('clerk.updateShippingInstruction', $transfer->shipping_id) }}"><span class="fa-regular fa-thumbs-up"></span></a>
                                            @else
                                                <a class="link dark__text-warning" data-bs-toggle="tooltip" data-bs-placement="left" title="SI pending confirmation and shipping"><span class="fa-regular fa-hourglass-half"></span></a>
                                            @endif
                                        ...
                                        <div class="dropdown font-sans-serif position-static" >
                                            ...
                                            <div class="dropdown-menu dropdown-menu-end border py-0">
                                                <div class="py-2">
                                                    ...
                                                    <a class="dropdown-item text-primary" href="{{ route('clerk.downloadSIDocument', $transfer->shipping_id) }}">Download SI</a>
                                                    <a class="dropdown-item text-dark" href="{{ route('clerk.downloadDriverClearance', $transfer->shipping_id) }}"> Driver Clearance</a>
                                                </div>
                                            </div>
                                        </div>
                                        @else
                                            @if($transfer->status == 0)
                                                @if($transfer->location_id == auth()->user()->station->location->location_id && auth()->user()->role_id == 3)
                                                    <a class="link text-warning"  onclick="return confirm('Are you sure you want to initiate Blend?')" data-bs-toggle="tooltip" data-bs-placement="left" title="Click to initiate Blend" href="{{ route('clerk.updateBlendSheet', $transfer->shipping_id) }}"><span class="fa-solid fa-share-from-square" ></span></a>
                                                @else
                                            ...
                                            @elseif($transfer->status == 1)
                                                @if($transfer->location_id == auth()->user()->station->location->location_id && auth()->user()->role_id == 3)
                                                    <a class="link text-info" data-bs-placement="left" title="Click to update OutTurn Report" data-bs-toggle="tooltip" href="{{ route('clerk.updateOutTurnReport', $transfer->shipping_id) }}"><span class="fa-solid fa-pen-to-square"></span></a>
                                                @else
                                            ...
                                            @elseif($transfer->status == 2)
                                                @if($transfer->location_id == auth()->user()->station->location->location_id && auth()->user()->role_id == 3)
                                                    <a class="link text-danger" data-bs-toggle="tooltip" data-bs-placement="left" title="Send Blend for approval" onclick="return confirm('Are you sure you want to send this Blend for approval?')" href="{{ route('clerk.markBlendTeaAsShipped', $transfer->shipping_id) }}"><span class="fa-regular fa-paper-plane"></span></a>
                                                @else
                                            ...
                                            <div class="dropdown font-sans-serif position-static" >
                                                ...
                                                <div class="dropdown-menu dropdown-menu-end border py-0">
                                                    <div class="py-2">
                                                        <a class="dropdown-item text-info" href="{{ route('clerk.addBlendTeas', $transfer->shipping_id) }}">View Blend Sheet</a>
                                                        <a class="dropdown-item text-primary" href="{{ route('clerk.downloadBlendSheet', $transfer->shipping_id) }}">Download Blend Sheet</a>
                                                        @if($transfer->status >= 3)
                                                            <a class="dropdown-item text-dark" href="{{ route('clerk.downloadOutturReport', $transfer->shipping_id) }}"> Download Outturn Report</a>
                                                        @endif
                                                        <a class="dropdown-item text-secondary" href="{{ route('clerk.downloadBlendDriverClearance', $transfer->shipping_id) }}"> Download Driver Clearance</a>
                                                    </div>
                                                </div>
                                            </div>
                                        @endif
