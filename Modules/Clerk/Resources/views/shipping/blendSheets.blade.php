                    ...
                    <table class="table mb-0 table-bordered table-striped fs-sm w-100" id="datatable">
                        ...
                        @foreach($sheets as $transfer)
                            ...
                            <tr>
                                ...
                                <td>
                                    {!! $transfer->status == 0 ? '<span class="badge bg-warning"> Blend Created </span>'
                                ...
                                @php
                                    $user = auth()->user();
                                    $canAccess =
                                        canuser('blend.create') ||
                                        (
                                            $transfer->location_id == $user->station->location->location_id
                                            && $user->role_id == 3
                                        );
                                    $canApprove = @canuser('blend.approve');
                                    $finalApproval = @canuser('blend.finalblendsheetapproval');
                                    $canManageLogistics = @canuser('blend.amendtransportdetails');
                                    $canShipBlend = @canuser('blend.markasshipped');
                                @endphp
                                <td nowrap="">
                                    <div class="d-flex align-items-center">
                                        @if($transfer->status == 0)
                                            @if($canAccess)
                                                <a class="link text-warning" onclick="return confirm('Are you sure you want to initiate this Blend?')" data-bs-toggle="tooltip" data-bs-placement="left" title="Click to initiate Blend" href="{{ route('clerk.initiateBlendSheet', $transfer->blend_id) }}"><span class="fa-solid fa-share-from-square"></span></a>
                                            @else
                                                <a class="link text-info" data-bs-toggle="tooltip" data-bs-placement="left" title="Blend pending initiation"><span class="fa-solid fa-spinner"></span></a>
                                            @endif
                                        @elseif($transfer->status == 1)
                                            @if($canApprove)
                                                <a class="link text-warning" data-bs-toggle="tooltip" data-bs-placement="left" title="Click to give first approval" onclick="return confirm('Are you sure you want to approve this Blend?')" href="{{ route('clerk.approveBlendFirst', $transfer->blend_id) }}"><span class="fa-regular fa-thumbs-up"></span></a>
                                            @else
                                                <a class="link text-info" data-bs-toggle="tooltip" data-bs-placement="left" title="Blend pending first approval"><span class="fa-regular fa-hourglass-half"></span></a>
                                            @endif
                                        @elseif($transfer->status == 2)
                                            @if($canAccess)
                                                <a class="link text-info" data-bs-placement="left" title="Click to complete OutTurn Report" data-bs-toggle="tooltip" href="{{ route('clerk.updateOutTurnReport', $transfer->blend_id) }}"><span class="fa-solid fa-pen-to-square"></span></a>
                                            @else
                                        ...
                                        @elseif($transfer->status == 3)
                                            @if($finalApproval)
                                                <a class="link text-danger" data-bs-toggle="tooltip" data-bs-placement="left" title="Approve Outturn Report" onclick="return confirm('Are you sure you want to approve Blend Outturn Report?')" href="{{ route('clerk.approveBlendFinal', $transfer->blend_id) }}"><span class="fa-regular fa-thumbs-up"></span></a>
                                            @else
                                        ...
                                        @elseif($transfer->status == 4 && ($transfer->blend_shipped ==null))
                                            @if($transfer->release_status == 1)
                                                @if($finalApproval && $canShipBlend)
                                                    <a class="link text-success" data-bs-toggle="tooltip" data-bs-placement="left" title="Mark Blend as Shipped" onclick="return confirm('Are you sure you want to mark this Blend as shipped?')" href="{{ route('clerk.markBlendShipped', $transfer->blend_id) }}"><span class="fa-solid fa-truck-ramp-box"></span></a>
                                                @else
                                        ...
                                        <div class="dropdown font-sans-serif position-static">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-end border py-0">
                                                <div class="py-2">
                                                    ...
                                                    @if($user->role_id == 2 && $transfer->status <= 4 || @canuser('blend.edit') && $transfer->status <= 4)
                                                        <a class="dropdown-item text-warning" href="{{ route('clerk.editBlendSheet', $transfer->blend_id) }}">Edit Blend Sheet</a>
                                                    @endif
                                                    @if(@canuser('blend.amendOutturn') && $transfer->status >= 3 && $transfer->status <= 4)
                                                    ...
                                                    @if($transfer->status >= 3)
                                                        <a class="dropdown-item text-primary" href="{{ route('clerk.downloadBlendSheet', $transfer->blend_id) }}" target="_blank">Blend Sheet</a>
                                                    ...
                                                    @if($transfer->status >= 4)
                                                        <a class="dropdown-item text-secondary" href="{{ route('clerk.downloadBlendPackingList', base64_encode($transfer->blend_id.":".$transfer->package_type)) }}" target="_blank"> Packing List</a>
                                                        <a class="dropdown-item text-secondary" href="{{ route('clerk.downloadBlendPackingListExcel', base64_encode($transfer->blend_id.":".$transfer->package_type)) }}" target="_blank"> Packing List (Excel)</a>
                                                        <a class="dropdown-item text-secondary" href="{{ route('clerk.downloadBlendPackingListCont', base64_encode($transfer->si_number.":".$transfer->package_type)) }}" target="_blank"> Packing List (Cont.)</a>
                                                        <a class="dropdown-item text-secondary" href="{{ route('clerk.downloadBlendPackingListContExcel', base64_encode($transfer->si_number.":".$transfer->package_type)) }}" target="_blank"> Packing List (Cont.) (Excel)</a>
                                                    @endif
                                                </div>
                                            </div>
                                        </div>
...
<script>
    ...
    document.addEventListener('show.bs.modal', function (event) {
        ...
        fetch(`{{ url('clerk/blend/logistics') }}/${blendId}`, {
