        ...
        <div class="card-header">
            <div class="row flex-between-center">
                ...
                <div class="col-6 col-sm-auto ms-auto text-end ps-0">
                    <div id="table-simple-pagination-replace-element">
                        @if($bs->status == 0 && @canuser('rebag.update') || @canUser('rebag.addblendteas') || @canuser('rebag.amend'))
                ...
                <div class="modal-content border-0">
                    ...
                    <div class="modal-body p-0">
                        <div class="rounded-top-3 bg-body-tertiary py-3 ps-4 pe-6">
                            <h5 class="mb-1" id="staticBackdropLabel">Select teas to add to rebag job</h5>
                        </div>
                        <div class="p-4">
                            <form id="shippingForm" method="POST" action="{{ route('clerk.storeRebagJobTeas', $bs->blend_id) }}">
                                @csrf
            ...
            <div class="modal-dialog modal-xl mt-6" role="document">
                <div class="modal-content border-0">
                    ...
                    <div class="modal-body p-0">
                        <div class="rounded-top-3 bg-body-tertiary py-3 ps-4 pe-6">
                            <h5 class="mb-1" id="staticBackdropLabel">Select blend balances to add to rebag job</h5>
                        </div>
                        <div class="p-4">
                            <form id="shippingForm" method="POST" action="{{ route('clerk.addRebagJobBlendBalanceTeas', $bs->blend_id) }}">
                                @csrf
                        ...
                        <table class="table mb-0 table-bordered table-striped" id="datatable">
                            ...
                            @foreach($teas as $shipping)
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if(@canuser('rebag.amend'))
                                            <a class="link-danger" data-bs-toggle="tooltip" data-bs-placement="left" title="Remove line from Rebag Job" onclick="return confirm('Are you sure you want to remove selected line from the SI?')" href="{{ route('clerk.deleteRebagJobTea', $shipping->blended_id) }}"><span class="fa fa-trash-alt"></span></a>
                                        @endif
                                    </td>
                                </tr>
                            @endforeach
                        ...
                        <ul class="list-unstyled mb-0">
                            ...
                            <li class="d-flex align-items-center fs-11 fw-medium pt-1 mb-3"><span class="dot bg-primary bg-opacity-50"></span>
                                <p class="lh-sm mb-0 text-700">Status :<span class="text-900 ps-2">
                                        {!! $bs->status == 0 ? '<span class="badge bg-warning"> Rebag Job Created </span>' : ($bs->status == 1 ? '<span class="badge bg-info"> Teas Updated </span>' : ($bs->status == 2 ? '<span class="badge bg-secondary"> Rebag Job Updated </span>' : ($bs->status == 3 ? '<span class="badge bg-dark"> Pend. Approval </span>' : '<span class="badge bg-success"> Shipped on'. \Carbon\Carbon::createFromTimestamp($bs->ship_date, config('app.timezone'))->format('D, d M Y H:i') .'</span>'))) !!}
                                    </span>
                                </p>
                            </li>
                        </ul>
