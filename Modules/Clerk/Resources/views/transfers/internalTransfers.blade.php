                    ...
                    <div id="table-simple-pagination-replace-element">
                        @if (@canuser('transfer.internal.create'))
                            <a class="btn btn-falcon-default btn-sm" data-bs-toggle="modal"
                                data-bs-target="#staticBackdrop"><span class="fas fa-plus"
                                    data-fa-transform="shrink-3 down-2"></span><span
                                    class="d-none d-sm-inline-block ms-1">New Request</span></a>
                        @endif
                    </div>
                </div>
                            ...
                            <div class="modal-body p-0">
                                ...
                                <div class="p-4">
                                    <form method="POST" id="myForm"
                                        action="{{ route('clerk.prepareInternalTransfer') }}">
                                        @csrf
                                        <div class="row row-cols-sm-3 g-2">
                                            ...
                                            @endphp
                                            <div class=" mb-4">
                                                <label class="fs-sm fw-bold my-2" style="font-size: 85% !important;">
                                            ...
                                            <div class=" mb-4">
                                                ...
                                                <select name="station" class="form-select" id="selectStation"
                                                    style="height: 57% !important;">
                                                    <option disabled selected>-- select client --</option>
                                                </select>
                                            </div>
                                            <div class=" mb-4">
                                                <label class="fs-sm fw-bold my-2" style="font-size: 85% !important;"> CLIENT
                                                    NAME</label>
                                                <select name="client" class="form-select" id="selectClients"
                                                    style="height: 57% !important;">
                                                    <option disabled selected>-- select client --</option>
                                                </select>
                                            </div>
                                        ...
                                        <div class="d-flex justify-content-center mt-1">
                                            <button id="submitButton" type="submit" class="btn btn-success col-8">PREPARE
                                    ...
                                    <td nowrap="">
                                        <div class="d-flex align-items-center">
                                            @if ($transfer->status === null)
                                                @if (@canuser('transfer.internal.create'))
                                                    <a class="link text-warning" data-bs-toggle="tooltip" data-bs-placement="left"
                                                       title="Click to initiate this transfer"
                                            ...
                                            @elseif($transfer->status == 0)
                                                @if (@canuser('transfer.internal.approve'))
                                                    <a class="link text-warning" data-bs-toggle="tooltip" data-bs-placement="left"
                                                       ...
                                                       href="{{ route('clerk.approveTransferFirst', base64_encode($transfer->delivery_number)) }}">
                                            ...
                                            @elseif($transfer->status == 1)
                                                @if (@canuser('transfer.internal.release') || @canuser('transfer.internal.release') && $transfer->origin == auth()->user()->station->location->location_id)
                                                ...
                                                @else
                                                    <a class="link text-danger" data-bs-toggle="tooltip" data-bs-placement="left" title="Approved, pending release">
                                                        <span class="fa-solid fa-check"></span>
                                                    </a>
                                                @endif
                                            @elseif($transfer->status == 2)
                                                @if (@canuser('transfer.internal.receive'))
                                                    <a class="link text-secondary" data-bs-toggle="tooltip" data-bs-placement="left"
                                                       title="Released — click to receive"
                                                       href="{{ route('clerk.prepareToReceiveTransfer', base64_encode($transfer->delivery_number)) }}">
                                                        <span class="fa-solid fa-truck-arrow-right"></span>
                                                    </a>
                                                @else
                                                    <a class="link text-secondary" data-bs-toggle="tooltip" data-bs-placement="left" title="Released, pending receipt">
                                                        <span class="fa-solid fa-truck-arrow-right"></span>
                                                    </a>
                                                @endif
                                            @elseif($transfer->status == 3)
                                                @if (@canuser('transfer.internal.approve.final'))
                                                    <a class="link text-primary" data-bs-toggle="tooltip" data-bs-placement="left"
                                                       ...
                                                       href="{{ route('clerk.approveTransferFinal', base64_encode($transfer->delivery_number)) }}">
                                                        <span class="fa-solid fa-check"></span>
                                                    </a>
                                                @else
                                                    <a class="link text-primary" data-bs-toggle="tooltip" data-bs-placement="left" title="Received, pending final approval">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-end border py-0">
                                                    <div class="py-2">
                                                        <a class="dropdown-item text-info"
                                                           href="{{ route('clerk.viewInternalTransferDetails', base64_encode($transfer->delivery_number)) }}">View Transfer</a>
                                                        @php $approved = \Modules\Clerk\Entities\Approval::where('job_id', $transfer->delivery_number)->get(); @endphp
                                                        @if ($approved && $approved->count() >= 1)
                                                            <a class="dropdown-item text-primary"
                                                               href="{{ route('clerk.downloadInterDelNote', base64_encode($transfer->delivery_number)) }}"
                                                               target="_blank"> Download Transfer </a>
                                                        @endif
                                                        @if ($transfer->status == 2 && $transfer->location_id == auth()->user()->station->location->location_id && @canuser('transfer.internal.receive'))
                                                            <a class="dropdown-item text-danger"
                                                               href="{{ route('clerk.prepareToReceiveTransfer', base64_encode($transfer->delivery_number)) }}">Receive Transfer</a>
                                                        @endif
                                                    </div>
                                                </div>
                                            </div>
        ...
        $('#selectWarehouse').change(function() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $('#selectStation').append(
                        '<option value="" selected disabled class="text-center"> -- select client --'
                    );

                    response.forEach(function(client) {
                        $('#selectStation').append('<option value="' + client
                            .station_id + '">' + client.station_name +
                            '</option>');
                    })
                }
            })
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $.each(response, function(index, client) {
                        $('#selectClients').append('<option value="' + client
                            .client_id + '">' + client.client_name + '</option>'
                        );
                    });
