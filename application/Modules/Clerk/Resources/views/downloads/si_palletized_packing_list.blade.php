...
{{--            <td>{{ $order->seal_number }}</td>--}}
            <?php
                ...
                $totalTare += (float) $order->package_tare * (int) $order->shipped_packages;
                $grossWeight += (float) str_replace([',', '.00'], '', $order->shipped_weight)
                    + ((float) $order->package_tare * (int) $order->shipped_packages)
                    + (float) $order->pallet_weight;
                $totalPalletW += $order->pallet_weight;
            ?>
        @endforeach
    </tbody>-->
    <tr class="subtotal-row tfooter" style="font-weight: bold;">
