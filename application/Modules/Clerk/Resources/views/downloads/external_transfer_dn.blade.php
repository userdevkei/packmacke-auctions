...
@php
$type = null;
if(in_array(auth()->user()->role_id, [1, 2, 4])){ $type = 'RELEASE NOTE'; }else{ $type = 'LOCAL DELIVERY NOTE';}
@endphp
<div class="header">{{ $type }}<hr></div>
<table>
    <tr>
        <td style="width: 70% !important;"> CLIENT NAME : <strong> {{ $details->buyer_name }} </strong></td>
...
<table>
    <tr>
        <td style="width: 40% !important;"> LOADING INSTRUCTION :<strong> {{ $details->loading_number }} </strong></td>
        <td style="width: 60% !important;"> WAREHOUSE : <b> {{ $details->warehouse_name }} </b> </td>
    </tr>
</table>
<br>
<table class="table">
