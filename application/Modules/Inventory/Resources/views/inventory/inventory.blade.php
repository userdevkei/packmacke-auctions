        ...
        <div class="card-header">
            <div class="row flex-between-center">
                <div class="col-6 col-sm-auto d-flex align-items-center pe-0">
                    <h5 class="fs-9 mb-0 text-nowrap py-0 py-xl-0">Inventory Balances</h5>
                </div>
...
{{--                        <input type="date" id="dateTo" name="dateTo" value="{{ $dateTo !== null ? ' ' : \Carbon\Carbon::parse($dateTo)->format('Y-d-m') }}">--}}
{{--                    </div>--}}
                    ...
                    <div class="form-group">
                        ...
                        <select id="item" name="item">
                            ...
                            @foreach($items as $itemOption)
                                <option value="{{ $itemOption->item_id }}" @selected($item == $itemOption->item_id)>
                    ...
                    <a href="{{ route('instock.view') }}" class="btn-reset btn-danger">Reset</a>
                    <button type="submit" name="export" value="1" class="btn-export btn-secondary">Export</button>

                    {{--                    <button type="button" class="btn-export btn-secondary">Export</button>--}}
                </form>
            </div>
                    ...
                    <table class="table mb-0 table-bordered table-striped" id="datatable">
                        ...
                        @foreach($inventories as $inventory)
                            <tr>
                                ...
                                <td> {{ ucwords(strtolower($inventory->item_name)) }} </td>
                                <td> {{ $inventory->unit }} </td>
                                <td> {{ $inventory->total_received }} </td>
                                <td> {{ $inventory->total_issued }} </td>
                                <td> {{ $inventory->current_balance }} </td>
                                <td> {{ $inventory->last_transaction_date }} </td>
                                <td>
                                    <a class="link link-dark" href="{{ route('inventory.client.summary', $inventory->client_id) }}"><i class="fa fa-eye"></i> </a>
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
